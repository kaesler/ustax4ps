...
import Prelude (class Show, bind, max, pure, show, unit, ($), (*), (+), (<>))
import UnsafeDates (unsafeMakeYear)

newtype BoundRegime = BoundRegime
  { regime :: Regime
  , year :: Year
...
standardDeduction (BoundRegime br) birthDate =
  br.unadjustedStandardDeduction <>
    ( if isAge65OrOlder birthDate br.year then
        br.adjustmentWhenOver65
          <>
            ( if isUnmarried br.filingStatus then br.adjustmentWhenOver65AndSingle
              else noMoney
            )
      else noMoney
    )
...
boundRegimeForFutureYear r y annualInflationFactor fs =
  let
    _ = if YV.yearIsFuture y then unit else unsafeThrow $ "Year must be in the future: " <> show y
    baseValues = YV.mostRecentForRegime r :: YearlyValues
